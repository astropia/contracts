...
contract ERC1155MixedFungible is ERC1155 {
    ...
    function safeTransferFrom(address _from, address _to, uint256 _id, uint256 _value, bytes calldata _data) override external {
        ...
        if (isNonFungible(_id)) {
            _transferNonFungibleToken(_from, _to, _id, _value);
        } else {
    ...
    function safeBatchTransferFrom(address _from, address _to, uint256[] calldata _ids, uint256[] calldata _values, bytes calldata _data) override external {
        ...
        for (uint256 i = 0; i < _ids.length; ++i) {
            ...
            if (isNonFungible(id)) {
                _transferNonFungibleToken(_from, _to, id, value);
            } else {
    ...
    function balanceOf(address _owner, uint256 _id) override external view returns (uint256) {
        if (isNonFungibleItem(_id))
            return _nfOwners[_id] == _owner ? 1 : 0;
    ...
    function balanceOfBatch(address[] calldata _owners, uint256[] calldata _ids) override external view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < _owners.length; ++i) {
            ...
            if (isNonFungibleItem(id)) {
                balances_[i] = _nfOwners[id] == _owners[i] ? 1 : 0;
            } else {
    ...
    function _transferNonFungibleToken(address _from, address _to, uint256 _id, uint256 _value) internal {
        require(_value == 1);
        ...
        if (_from != address(0)) {
            ...
            fromTokens[_nfIndex[_id]] = fromTokens[fromTokens.length - 1];
            fromTokens.pop();
        }
        uint256[] storage toTokens = _nft[_to][baseType];
        _nfIndex[_id] = toTokens.length;
        toTokens.push(_id);
